...
impl App { //создание App
    pub fn new(config: &AppConfig, _cc: &eframe::CreationContext<'_>) -> Self {
    Self {
        dashboard: dashboard::Dashboard::new(config), //Создаёт окно настроек, передавая туда загруженную конфигурацию
        config_window: config_window::ConfigWindow::new(config.clone()), // Тут clone(), если нужно сохранить отдельную копию
    }
}
